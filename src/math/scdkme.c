 ...
 *
 * Faithful port of recycle/inteCenV.h and recycle/vtype.h.
 */
...
void scdk_vargs_from_model(scdk_vargs_t *varg, const argsGIModel_t *model, model_type_t mtype)
{
    ...
    varg->mc = model->mc;
    varg->mb = model->mb;
    varg->alpha[0] = GI_ALPHA_K[0];
    varg->alpha[1] = GI_ALPHA_K[1];
    varg->alpha[2] = GI_ALPHA_K[2];
    varg->gamma[0] = GI_GAMMA_K[0];
    varg->gamma[1] = GI_GAMMA_K[1];
    varg->gamma[2] = GI_GAMMA_K[2];
    varg->b = model->b;
    varg->c = model->c;
    varg->sigma0 = model->sigma_0;
    varg->s = model->s;
    varg->f = 0.5;
    varg->mu = model->mu;
    varg->econt = model->epsilon_cont;
    varg->etens = model->epsilon_tens;
    varg->esov = model->epsilon_sov;
    varg->esos = model->epsilon_sos;
    varg->eCoul = 0.0;
    varg->model = mtype;
}


typedef double (*vCenPart)(double,basis_qnum,basis_qnum,scdk_vargs_t,int);
...
double Ti(double p,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mc;
	switch(c)
	{
...
double InteCenV(double b11,int n,basis_qnum qf,basis_qnum qi,vCenPart v,scdk_vargs_t varg,int c)
{
...
double inteVcenPart(txrp_vtype get_trxp_vtype,sumckdk_scdk scdk,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,vCenPart v,int c)
{
	int i,j;
	double p,sum;
	double t[21];
...
typedef double (*inteCenV)(double b11,int n,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c);

double inteVcenPartA(txrp_vtype get_trxp_vtype,sumckdk_scdk scdk,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,inteCenV iv,int c)
{
	int i,j;
	double p,sum;
	double t[21];
...
/* Baryon pair colour Cij = −2/3, OCent = 1. Matches recycle c/2 on the constant. */
static double baryon_vconf_r(double r, basis_qnum qf, basis_qnum qi, scdk_vargs_t varg, int c)
{
	double mi, mj, mk;
	getijk(qf.m1, qf.m2, qf.m3, &mi, &mj, &mk, c);
	double sigmaij = getsigmaij(varg.sigma0, varg.s, mi, mj);
	double Cij = -2.0 / 3.0;
	(void)qi;
	if (r == 0.0) {
		return -0.75 * Cij * varg.c;
	}
	if (varg.model == MODEL_GISCREEN && varg.mu > 0.0) {
		double sig2 = sigmaij * sigmaij;
		double mu = varg.mu;
		double mu2_4sig2 = mu * mu / (4.0 * sig2);
		double mu_m = mu - 2.0 * r * sig2;
		double mu_p = mu + 2.0 * r * sig2;
		double pref = -3.0 * Cij * varg.b * exp(-r * mu) / (16.0 * r * mu * sig2);
		double inner1 = 4.0 * r * sig2 * exp(r * mu);
		double inner2 = mu_m * exp(mu2_4sig2) * erfc(mu_m / (2.0 * sigmaij));
		double inner3 = mu_p * exp(mu2_4sig2 + 2.0 * r * mu) * erfc(mu_p / (2.0 * sigmaij));
		return pref * (inner1 + inner2 - inner3) - 0.75 * Cij * varg.c;
	}
	double rsig = r * sigmaij;
	double pref = -3.0 * Cij * varg.b / (8.0 * r * sigmaij * sigmaij);
	double inner1 = (2.0 / sqrt(3.1415926535897932385)) * r * sigmaij * exp(-rsig * rsig);
	double inner2 = (1.0 + 2.0 * rsig * rsig) * erf(rsig);
	return pref * (inner1 + inner2) - 0.75 * Cij * varg.c;
}

static double baryon_vsosii_r(double r, basis_qnum qf, basis_qnum qi, scdk_vargs_t varg, int c)
{
	double mi, mj, mk;
	getijk(qf.m1, qf.m2, qf.m3, &mi, &mj, &mk, c);
	if (r == 0.0) {
	...
	double dV = (baryon_vconf_r(r + h, qf, qi, varg, c) - baryon_vconf_r(r - h, qf, qi, varg, c)) / (2.0 * h);
	return -dV / (2.0 * r * mi * mi);
}

static double baryon_vsosjj_r(double r, basis_qnum qf, basis_qnum qi, scdk_vargs_t varg, int c)
{
	double mi, mj, mk;
	getijk(qf.m1, qf.m2, qf.m3, &mi, &mj, &mk, c);
	if (r == 0.0) {
...
double inteVstring(double b11,int n,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	...
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
	if (varg.model == MODEL_GISCREEN && varg.mu > 0.0) {
		return InteCenV(b11, n, qf, qi, baryon_vconf_r, varg, c);
	}
...
double inteVsosii(double b11,int n,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	...
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
	if (varg.model == MODEL_GISCREEN && varg.mu > 0.0) {
		return InteCenV(b11, n, qf, qi, baryon_vsosii_r, varg, c);
	}
	sigmaij=getsigmaij(varg.sigma0,varg.s,mi,mj);
...
double inteVsosjj(double b11,int n,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	...
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
	if (varg.model == MODEL_GISCREEN && varg.mu > 0.0) {
		return InteCenV(b11, n, qf, qi, baryon_vsosjj_r, varg, c);
	}
	sigmaij=getsigmaij(varg.sigma0,varg.s,mi,mj);
...
double pogeG(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double pcont(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double ptens(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double psovii(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double psovjj(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double psovji(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double psovij(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double psosii(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
double psosjj(double r,basis_qnum qf,basis_qnum qi,scdk_vargs_t varg,int c)
{
	double mi,mj,mk;
	getijk(qf.m1,qf.m2,qf.m3,&mi,&mj,&mk,c);
...
void sumckdk_scdk_vtype(sumckdk_scdk *scdk,vtype vsodt,int fg,basis_list qnlist,matrix_t **mlsj,int nf,int nfp,int ni,int nip,pthread_mutex_t *mutex,int lock,int c)
{
	...
	for(f=0;f<mlsj[nf][nfp].row;f++)
	{
		for(i=0;i<mlsj[ni][nip].row;i++)
		{
			coef=     mlsj[nf][nfp].value[f][0];
			msa1=     mlsj[nf][nfp].value[f][1];
			msa2=     mlsj[nf][nfp].value[f][2];
			msa3=     mlsj[nf][nfp].value[f][3];
		 	ml1 =(int)mlsj[nf][nfp].value[f][4];
			ml2 =(int)mlsj[nf][nfp].value[f][5];

			coei=     mlsj[ni][nip].value[i][0];
			msb1=     mlsj[ni][nip].value[i][1];
			msb2=     mlsj[ni][nip].value[i][2];
			msb3=     mlsj[ni][nip].value[i][3];
		 	ml3 =(int)mlsj[ni][nip].value[i][4];
			ml4 =(int)mlsj[ni][nip].value[i][5];
...
void vsoii(sumckdk_scdk *scdk,double coef,double coei,double msai,double msaj,double msak,double msbi,double msbj,double msbk,int l1,int l2,int l3,int l4,int ml1,int ml2,int ml3,int ml4,basis_qnum qa,basis_qnum qb,pthread_mutex_t *mutex,int lock,int c)
{
	...
	if(msak==msbk)
	{
		for(M=-1;M<=1;M++)
			...
			for(mu=-1;mu<=1;mu++)
			{
				coe=clebsch_gordan(1,-M,1,M,0,0)*clebsch_gordan(1,mu,1,M,1,mu+M)*sqrt(si*(si+1))*clebsch_gordan(si,msbi,1,-M,si,msai);
				if(0!=coe)
				{
...
void vsojj(sumckdk_scdk *scdk,double coef,double coei,double msai,double msaj,double msak,double msbi,double msbj,double msbk,int l1,int l2,int l3,int l4,int ml1,int ml2,int ml3,int ml4,basis_qnum qa,basis_qnum qb,pthread_mutex_t *mutex,int lock,int c)
{
	...
	if(msak==msbk)
	{
		for(M=-1;M<=1;M++)
			...
			for(mu=-1;mu<=1;mu++)
			{
				coe=clebsch_gordan(1,-M,1,M,0,0)*clebsch_gordan(1,mu,1,M,1,mu+M)*sqrt(sj*(sj+1))*clebsch_gordan(sj,msbj,1,-M,sj,msaj);
				if(0!=coe)
				{
					sumckdk_scdk_calc(scdk,pow(-1,mu)*coe*coef*coei,l1,l2,l3,l4,1,1,ml1,ml2,ml3,ml4,-mu,mu+M,mutex,lock);
				}
