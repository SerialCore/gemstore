    ...
    */
    ...
    typedef struct
    {
        int l;                          /* This thread's index (0 … lth-1) */

        int *cal_pth;                   /* Global flag: 1 = keep running, 0 = terminate thread */

        pthread_mutex_t *mutex_subpro;  /* Per-thread mutex (protects condition variable & shared_data_subpro) */
        pthread_mutex_t *mutex_parent;  /* Shared mutex for parent–child synchronization */

        pthread_cond_t *cond_subpro;    /* Per-thread condition variable (worker waits here) */
        pthread_cond_t *cond_parent;    /* Parent waits here until all workers finish current task */

        int *shared_data_subpro;        /* Per-thread flag: 1 = work to do, 0 = waiting */
        int *shared_data_parent;        /* Parent flag: 1 = all threads finished current job */

        int *lth_use;                   /* How many threads are actually used in current parallel section */
        int *lth_cal;                   /* Counter: how many threads have reported completion */

        int *type;                      /* Command code – tells worker what operation to perform (1,2,3,4,5) */
        int *stList;                    /* Start column indices this thread should process */
        int *edList;                    /* End column indices this thread should process */
        double **a;                     /* The working matrix (usually the Householder reflectors or eigenvectors) */
        int *i;                         /* Current column/index being processed (shared) */
        double *uu;                     /* Householder scalars (used in rank-2 update – type 2) */
        double *beta;                   /* Householder scalars (used in rank-2 update – type 2) */
        int *jList;                     /* Batched Givens rotations (j) – used by type 5 */
        double *cList;                  /* Batched Givens rotations (cos) – used by type 5 */
        double *sList;                  /* Batched Givens rotations (sin) – used by type 5 */
        int *lx;                        /* Current number of batched rotations (index in jList/cList/sList) */
        double *vc;                     /* Temporaries for vector rotation (not always used) */
        double *vs;                     /* Temporaries for vector rotation (not always used) */
    } trid_args;
    ...
    typedef struct
    {
        ...
        int *shared_data_subpro;        /* Per-thread flag: 1 = work to do, 0 = waitings */
        int *shared_data_parent;        /* Parent flag: 1 = all threads finished current job */
        ...
        int *edList;                    /* End column indices this thread should process */
        double **a;                     /* The working matrix (usually the Householder reflectors or eigenvectors) */
        double **b;                     /* The second input matrix B (must be positive definite) */
        double **vt;                    /* Output eigenvectors (final result) */
        double **G;                     /* Cholesky factor L of B (lower triangular) */
        double **IG;                    /* Inverse of G (L⁻¹) */
        double **IGA;                   /* Intermediate: L⁻¹ A */
        double **S;                     /* Final reduced matrix: L⁻¹ A L⁻ᵀ  (tridiagonalized) */
        int *i;                         /* Current column/index being processed (shared) */
        int *ii;                        /* Loop variable for inverse computation (used in type 2) */
        int *j;                         /* Current column in Cholesky (used in type 1) */
        int n;                          /* Matrix dimension (copy – read-only) */
    ...
    /* Worker thread function for tridiagonal-related matrix updates */
    void *eigen_tridiagonal_fun(void *args);
    void *eigen_tridiagonal_fun(void *args)
    {
    ...
    /* Worker thread function for generalized (Cholesky-based) path */
    void *eigen_general_fun(void *args);
    void *eigen_general_fun(void *args)
    {
    ...
    void eigen_tridiagonal_thread(double **a, int n, double *d, double *e, double *et, int lt)
    {
        ...
        lth_use = lth;
        for (l = 0; l < lth; l++) {
            pthread_create(&tid[l], NULL, eigen_tridiagonal_fun, &arg[l]);
        }
    ...
    void eigen_general_thread(double **a, double **b, int n, double *d, double **vt, int lt)
    {
        ...
        lth_use = lth;
        for (l = 0; l < lth; l++) {
            pthread_create(&tid[l], NULL, eigen_general_fun, &arg[l]);
        }
