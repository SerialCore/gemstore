...
void sumckdk_scdk_vtype(sumckdk_scdk *scdk,vtype vsodt,int fg,basis_list qnlist,matrix **mlsj,int nf,int nfp,int ni,int nip,pthread_mutex_t *mutex,int lock,int c)
{
...
void vcont(sumckdk_scdk *scdk,double coef,double coei,double msai,double msaj,double msak,double msbi,double msbj,double msbk,int l1,int l2,int l3,int l4,int ml1,int ml2,int ml3,int ml4,basis_qnum qa,basis_qnum qb,pthread_mutex_t *mutex,int lock,int c)
{
	...
	if(msak==msbk)
	{
		for(M=-1;M<=1;M++)
        	{
			coe=coe-sqrt(3)*sqrt(si*(si+1))*sqrt(sj*(sj+1))*clebschGordan(1,M,1,-M,0,0)*clebschGordan(si,msbi,1,M,si,msai)*clebschGordan(sj,msbj,1,-M,sj,msaj);
        	}
	}
	if(0!=coe)
	{
...
void vtens(sumckdk_scdk *scdk,double coef,double coei,double msai,double msaj,double msak,double msbi,double msbj,double msbk,int l1,int l2,int l3,int l4,int ml1,int ml2,int ml3,int ml4,basis_qnum qa,basis_qnum qb,pthread_mutex_t *mutex,int lock,int c)
{
	...
	if(msak==msbk)
	{
		for(M=-2;M<=2;M++)
			...
			for(mu=-1;mu<=1;mu++)
			{
				coe=clebschGordan(2,-M,2,M,0,0)*clebschGordan(1,mu,1,-M-mu,2,-M)*sqrt(si*(si+1))*sqrt(sj*(sj+1))*clebschGordan(si,msbi,1,mu,si,msai)*clebschGordan(sj,msbj,1,-M-mu,sj,msaj);
				if(0!=coe)
				{
